...
完整 `.dm3` / `.dm4` metadata parity 不是第一版需求。
...
- Unsupported files 會 skipped；如果 batch 被取消，remaining files 會標成
  cancelled。On-screen status list 只顯示 image inputs，不顯示 `.txt`、
  `.csv`、`.json` 這類非影像檔案。
